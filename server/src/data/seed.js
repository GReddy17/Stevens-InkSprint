...
const seed = async () => {
	try {
		...
		const votingTypes = ['EVERYONE', 'JUDGES', 'CREATOR']

		const now = Date.now()
		const day = 24 * 60 * 60 * 1000

		function getContestDates(index) {
			const datePatterns = [
				// UPCOMING
				{
			...
			return datePatterns[index % datePatterns.length]
		}
		...
		for (let i = 0; i < contestTemplates.length; i++) {
			...
			const { startTime, endTime } = getContestDates(i)
			const votingType = votingTypes[i % votingTypes.length]
			...
			const contest = await Contest.create({
				...
				votingGroupMemberIds:
					votingType === 'JUDGES' ? [users[3]._id, users[4]._id] : [],
